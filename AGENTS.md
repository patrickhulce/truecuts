...
- A member face is the two spanning axes (earlier dimension first: L, then W, then T) plus `@0` or `@1`. `@0` is the plane through the origin; `@1` is the plane at the stock extent of the other axis. `LxW@0` / `LxW@1` are the wide faces (normal −Y / +Y), `LxT@0` / `LxT@1` the edges (normal −Z / +Z), `WxT@0` / `WxT@1` the ends (normal −X / +X). A crosscut does not move these planes; the cut cap is not a face id.
