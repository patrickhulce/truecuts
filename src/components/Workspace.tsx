...
export function Workspace() {
  const { text, setText, compiled, reset } = useDocument();
    ...
    <div className="flex h-full min-h-0 flex-col">
      <header className="flex items-center justify-between border-b border-[#3d2a18] bg-[#241a10] px-4 py-2.5">
        ...
        <div className="flex items-center gap-4 text-xs text-[#a89070]">
          ...
          <button
            type="button"
            onClick={reset}
            className="rounded border border-[#6b4a2b] bg-[#1a120b] px-2.5 py-1 text-[#d6c3a3] hover:border-[#f59e0b] hover:text-[#f59e0b]"
          >
            Reset demo
          </button>
        </div>
      ...
      <Group orientation="horizontal" className="min-h-0 flex-1" defaultLayout={{ editor: 40, viewport: 60 }}>
        ...
        <Panel id="viewport" minSize={30}>
          <Viewport scene={compiled.scene} selectedKey={selectedKey} onSelect={setSelectedKey} />
        </Panel>
      </Group>
    </div>
  );
